...
if($errors) {
    ...
    $sql = "INSERT INTO staff (staffid, surname, othernames, region, district, loc, posaddrs, digaddrs, nid, id_type) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?);";

    $success = $db->insertRow($sql, [$staffid, $surname, $othernames, $region, $district, $loc, $posaddrs, $digaddrs, $nid, $id_type]);

    if($success) {
        header('Location: userlogindetails.php?id=' . $staffid);
    }
}
